...
function renderDynamicYears() {
    ...
    VALID_YEARS.forEach(yearName => {
        ...
        const pendingUpdate = hasDownload ? pendingYearSubjectUpdates.get(yearName) : null;
...
async function fetchRepositorySubjectIndex() {
    ...
    tree.tree.forEach(item => {
        if (item.type !== 'blob' || !item.path?.toLowerCase().endsWith('.json')) return;
        if (item.path.split('/').pop() === 'myOwnDic.json') return;

        const rootFolder = item.path.split('/')[0];
        const yearName = VALID_YEARS.find(year =>
...
function getLocallyKnownSubjectPaths(yearName) {
    ...
    try {
        const subjects = JSON.parse(localStorage.getItem(getYearDataKey(yearName)) || '[]');
        ...
        console.warn(`Could not inspect the saved subjects for ${yearName}:`, error);
    }
    return paths;
}
...
function refreshYearUpdateControls() {
    ...
    yearsContainer.querySelectorAll('.offline-year-card').forEach(card => {
        const yearName = card.querySelector('.year-card-main strong')?.textContent?.trim();
        if (!VALID_YEARS.includes(yearName) || !card.classList.contains('downloaded-year')) return;
        ...
        if (hasNewSubjects && !updateButton && actions) {
            ...
            updateButton.addEventListener('click', () => openDownloadModal(yearName));
            actions.insertBefore(updateButton, actions.querySelector('.year-remove-btn'));
        ...
        if (updateButton && hasNewSubjects) {
            const label = update.count === 1 ? 'تحديث جديد' : `تحديث (${update.count})`;
            if (updateButton.textContent.trim() !== label) updateButton.textContent = label;
        ...
        if (hasNewSubjects && summary && !existingNotice) {
            ...
            notice.textContent = update.count === 1 ? ' · مادة جديدة' : ` · ${update.count} مواد جديدة`;
    ...
    yearUpdateCheckPromise = (async () => {
        try {
            ...
            downloadedYears.forEach(yearName => {
                ...
                const addedFiles = (remoteFilesByYear.get(yearName) || [])
                    .filter(file => !locallyKnownPaths.has(file.path));
...
function watchYearDownloadCompletion() {
    ...
    const timer = window.setInterval(() => {
        ...
        if (afterDownload && afterDownload !== beforeDownload && !downloadInProgress) {
            window.clearInterval(timer);
            pendingYearSubjectUpdates.delete(yearName);
            refreshYearUpdateControls();
            checkDownloadedYearsForNewSubjects(true);
        } else if (timedOut || (!downloadInProgress && document.getElementById('download-modal')?.classList.contains('hidden'))) {
            window.clearInterval(timer);
        }
    }, 400);
}
...
function bindYearUpdateNotifications() {
    ...
    document.getElementById('download-confirm-btn')
        ?.addEventListener('click', watchYearDownloadCompletion);
    window.addEventListener('online', () => checkDownloadedYearsForNewSubjects(true));
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible') checkDownloadedYearsForNewSubjects();
    });
    window.setInterval(() => {
        if (document.visibilityState === 'visible') checkDownloadedYearsForNewSubjects(true);
...
function refreshVocabularyCloudInterface() {
    ...
    if (usernameInput && !usernameInput.value) usernameInput.value = config.username;
    if (repoInput && !repoInput.value) repoInput.value = config.repo;
    if (tokenInput && !tokenInput.value) tokenInput.value = config.token;
    if (openButton) {
        openButton.textContent = config.enabled ? '☁️ المزامنة مفعّلة' : '☁️ تسجيل الدخول والمزامنة';
    }
    nowButton?.classList.toggle('hidden', !config.enabled);
    signOutButton?.classList.toggle('hidden', !config.enabled);

    if (config.enabled) {
        setVocabularyCloudStatus('متصل — تتم مزامنة الكلمات والملاحظات بين أجهزتك.', 'success');
    }
}
...
function getCloudTombstones() {
    ...
    return {
        vocabulary: stored?.vocabulary && typeof stored.vocabulary === 'object' ? stored.vocabulary : {},
...
function mergeTimestampMaps(first = {}, second = {}) {
    ...
    Object.entries(second || {}).forEach(([key, value]) => {
        if (cloudTimestamp(value) >= cloudTimestamp(merged[key])) merged[key] = value;
    });
    return merged;
}
...
function mergeCloudCollections(localData, remoteData) {
    ...
    const deleted = {
        vocabulary: mergeTimestampMaps(remote.deleted?.vocabulary, local.deleted?.vocabulary),
    ...
    [...(remote.vocabulary || []), ...(local.vocabulary || [])].forEach(entry => {
        ...
        if (!existing || cloudTimestamp(entry.updatedAt || entry.createdAt) >= cloudTimestamp(existing.updatedAt || existing.createdAt)) {
    ...
    [...(remote.notes || []), ...(local.notes || [])].forEach(note => {
        if (!note?.key?.startsWith('note_') || typeof note.value !== 'string') return;
        const existing = notesByKey.get(note.key);
        if (!existing || cloudTimestamp(note.updatedAt) >= cloudTimestamp(existing.updatedAt)) {
            notesByKey.set(note.key, note);
        }
    });

    const vocabulary = [...vocabularyById.values()]
        .filter(entry => cloudTimestamp(deleted.vocabulary[entry.id]) < cloudTimestamp(entry.updatedAt || entry.createdAt))
        .sort((a, b) => cloudTimestamp(b.updatedAt || b.createdAt) - cloudTimestamp(a.updatedAt || a.createdAt));
    const notes = [...notesByKey.values()]
        .filter(note => cloudTimestamp(deleted.notes[note.key]) < cloudTimestamp(note.updatedAt))
...
function cloudComparableData(data) {
    ...
    return JSON.stringify({
        vocabulary: [...(prepared.vocabulary || [])].sort((a, b) => String(a.id).localeCompare(String(b.id))),
        notes: [...(prepared.notes || [])].sort((a, b) => String(a.key).localeCompare(String(b.key))),
...
function applyCloudDataLocally(data) {
    ...
    localStorage.setItem(CLOUD_TOMBSTONES_KEY, JSON.stringify(data.deleted || { vocabulary: {}, notes: {} }));
...
function captureCloudDeletions() {
    ...
    const currentVocabularyIds = new Set(current.vocabulary.map(entry => entry.id));
    const currentNoteKeys = new Set(current.notes.map(note => note.key));

    cloudLastKnownSnapshot.vocabulary.forEach(entry => {
        if (!currentVocabularyIds.has(entry.id)) deleted.vocabulary[entry.id] = deleted.vocabulary[entry.id] || now;
    });
    cloudLastKnownSnapshot.notes.forEach(note => {
        if (!currentNoteKeys.has(note.key)) deleted.notes[note.key] = deleted.notes[note.key] || now;
    });
...
async function readVocabularyCloudFile(config) {
    ...
    try {
        ...
        throw new Error('The cloud sync file exists but is not valid UniQuiz data.');
    }
}

async function writeVocabularyCloudFile(config, data, sha = null) {
    const body = {
        message: sha ? 'Update UniQuiz words and notes' : 'Create UniQuiz words and notes',
        content: encodeCloudContent(JSON.stringify({
            ...data,
            updatedAt: new Date().toISOString()
        }, null, 2))
    };
...
function explainCloudSyncError(error) {
    ...
    if (error?.status === 403) return 'رمز الدخول لا يملك صلاحية قراءة وكتابة محتويات المستودع.';
    if (error?.status === 404) return 'لم يتم العثور على المستودع. أنشئه أولًا وتأكد من الاسم والصلاحيات.';
    if (error?.status === 409) return 'تغيّرت البيانات على جهاز آخر. حاول المزامنة مرة ثانية.';
    return error?.message || 'تعذرت المزامنة. تحقق من الاتصال وحاول مرة أخرى.';
}

async function syncVocabularyCloudData(showStatus = true) {
    const config = getVocabularyCloudConfig();
    if (!config.enabled || !config.username || !config.repo || !config.token || cloudSyncBusy) return false;
    if (!navigator.onLine) {
        if (showStatus) setVocabularyCloudStatus('أنت غير متصل بالإنترنت. بقيت بياناتك محفوظة على هذا الجهاز.', 'error');
        return false;
    }

    cloudSyncBusy = true;
    if (showStatus) setVocabularyCloudStatus('جاري مزامنة الكلمات والملاحظات…', 'loading');
    ...
    try {
        for (let attempt = 0; attempt < 2; attempt++) {
            ...
            if (!remoteFile.data || cloudComparableData(merged) !== cloudComparableData(remoteFile.data)) {
                try {
            ...
            refreshVocabularyCloudInterface();
            setVocabularyCloudStatus('تمت المزامنة بنجاح. بياناتك متاحة الآن على أجهزتك الأخرى.', 'success');
            return true;
        }
    } catch (error) {
        console.warn('Vocabulary cloud sync failed:', error);
        if (showStatus) setVocabularyCloudStatus(explainCloudSyncError(error), 'error');
        return false;
    } finally {
        cloudSyncBusy = false;
    }
...
async function connectVocabularyCloudSync() {
    const username = document.getElementById('vocabulary-sync-username')?.value.trim() || '';
    const repo = document.getElementById('vocabulary-sync-repo')?.value.trim() || '';
    const token = document.getElementById('vocabulary-sync-token')?.value.trim() || '';
    if (!username || !repo || !token) {
        setVocabularyCloudStatus('أدخل اسم المستخدم واسم المستودع ورمز الدخول.', 'error');
        return;
    }
...
function disconnectVocabularyCloudSync() {
    if (!confirm('هل تريد تسجيل الخروج من المزامنة على هذا الجهاز؟ ستبقى كلماتك وملاحظاتك المحلية محفوظة.')) return;
    localStorage.setItem('sync_enabled', 'false');
    localStorage.removeItem('sync_token');
    const tokenInput = document.getElementById('vocabulary-sync-token');
    if (tokenInput) tokenInput.value = '';
    refreshVocabularyCloudInterface();
    setVocabularyCloudStatus('تم تسجيل الخروج. بقيت البيانات محفوظة على هذا الجهاز.', '');
}
...
function bindVocabularyCloudSync() {
    ...
    document.addEventListener('click', event => {
        ...
        } else if (target.matches('.vocabulary-row-action.delete, [data-cloud-sync-change="notes"]')) {
            window.setTimeout(scheduleVocabularyCloudSync, 0);
...
const AUTOMATIC_UPDATE_PENDING_RELOAD_KEY = 'uniquiz_auto_update_pending_reload_v1';
...
function isAutomaticUpdateDue() {
    ...
    if (lastSuccess && now - lastSuccess < AUTOMATIC_UPDATE_INTERVAL_MS) return false;
    if (lastAttempt && now - lastAttempt < AUTOMATIC_UPDATE_RETRY_MS) return false;
    return true;
}
...
function showAutomaticUpdateNotice(message, type = 'success') {
    ...
    notice._hideTimer = window.setTimeout(() => notice.classList.remove('is-visible'), 6500);
}
...
async function updateDownloadedYearInBackground(yearName) {
    if (!localStorage.getItem(getYearDataKey(yearName))) {
        return { yearName, changedFileCount: 0, newSubjectCount: 0, removedSubjectCount: 0 };
    }
    ...
    for (const file of files) {
        ...
        try {
            ...
            const content = await response.json();
            const data = Array.isArray(content) ? content[0] : content;
            if (!data || !Array.isArray(data.questions)) throw new Error(`Invalid subject data in ${file.path}.`);
    ...
    const removedSubjectCount = existingSubjects.filter(subject => !remotePaths.has(subject.id)).length;
    const contentChanged = changedFileCount > 0 || removedSubjectCount > 0;
    if (!contentChanged) {
        return { yearName, changedFileCount: 0, newSubjectCount: 0, removedSubjectCount: 0, failedFileCount };
    }
    if (freshSubjects.length === 0) throw new Error(`No valid subjects remained in ${yearName}.`);

    const mergedSubjects = freshSubjects.map(freshSubject => {
        const existingSubject = existingById.get(freshSubject.id);
        if (!existingSubject || existingSubject === freshSubject) return freshSubject;
    ...
    const metadata = {
        ...
        subjectCount: cacheableData.length,
        questionCount: cacheableData.reduce((total, subject) => total + subject.questions.length, 0),
        sizeBytes: new Blob([serialisedData]).size,
    ...
    return { yearName, changedFileCount, newSubjectCount, removedSubjectCount, failedFileCount };
}

function sendMessageToServiceWorker(worker, message, timeoutMs = 12000) {
    if (!worker || typeof MessageChannel === 'undefined') return Promise.resolve({ ok: false, changed: false });
    return new Promise(resolve => {
        const channel = new MessageChannel();
        const timer = window.setTimeout(() => resolve({ ok: false, changed: false }), timeoutMs);
...
async function refreshApplicationShellInBackground() {
    ...
    try {
        ...
        const result = await sendMessageToServiceWorker(worker, { type: 'REFRESH_APP_SHELL' });
...
function reloadUpdatedApplicationWhenSafe() {
    ...
    try {
        reloadPending = sessionStorage.getItem(AUTOMATIC_UPDATE_PENDING_RELOAD_KEY) === '1';
    } catch (error) {
        return;
    }
    if (!reloadPending || currentSubject || automaticUpdateInProgress || document.visibilityState !== 'visible') return;
...
async function performAutomaticDailyUpdate(force = false) {
    ...
    localStorage.setItem(AUTOMATIC_UPDATE_LAST_ATTEMPT_KEY, new Date().toISOString());
    ...
    try {
        ...
        if (failedYearCount === 0) {
            localStorage.setItem(AUTOMATIC_UPDATE_LAST_SUCCESS_KEY, new Date().toISOString());
        }
        const changedFiles = results.reduce((total, result) => total + result.changedFileCount + result.removedSubjectCount, 0);
        if (changedFiles > 0) {
            const changedYears = results.filter(result => result.changedFileCount || result.removedSubjectCount).length;
...
function bindAutomaticDailyUpdates() {
    if ('serviceWorker' in navigator && location.protocol !== 'file:') {
        ...
        navigator.serviceWorker.addEventListener('message', event => {
            if (event.data?.type === 'RUN_DAILY_CONTENT_UPDATE') performAutomaticDailyUpdate();
    ...
    window.setInterval(() => performAutomaticDailyUpdate(), AUTOMATIC_UPDATE_INTERVAL_MS);
